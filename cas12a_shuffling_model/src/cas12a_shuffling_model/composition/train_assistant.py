...
from cas12a_shuffling_model.composition.chimera_repr import SLOT_COUNT, canonicalize_chimera_table, slot_columns
...
class _AssistantDataset(Dataset):
    def __init__(self, slots: np.ndarray, extra: np.ndarray, target: np.ndarray, exp: np.ndarray):
        self.slots = slots
        self.extra = extra
        self.target = target
        self.exp = exp
...
def _prepare_arrays(
    ...
    *,
    ...
    if train_idx is None or ex.shape[1] == 0:
        ...
        ex = (ex - mean) / std
    return slots, ex, y, exp, mean, std
...
def _run_epoch(
    *,
    ...
    stats = {"loss": 0.0, "top": 0.0, "corr": 0.0, "pair": 0.0, "exp": 0.0, "batches": 0}
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss = cfg.top_weight * l_top + cfg.corr_weight * l_corr + cfg.pair_weight * l_pair + cfg.experimental_weight * l_exp
            if is_train:
        ...
        stats["batches"] += 1
    if stats["batches"] > 0:
        for k in ("loss", "top", "corr", "pair", "exp"):
...
def train_assistant_ranker(
    *,
    ...
    slots, extra, y, exp, fmean, fstd = _prepare_arrays(
        df,
    ...
    ds = _AssistantDataset(slots, extra, y, exp)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Assistant epoch %d/%d train_loss=%.4f val_loss=%.4f global_corr=%.4f top5=%.4f",
            epoch,
            train_cfg.epochs,
            row["train_loss"],
            row["val_loss"],
